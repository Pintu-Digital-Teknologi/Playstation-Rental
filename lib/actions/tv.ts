...
// --- TV Control Action ---
export async function controlTVAction(
  tvId: string,
  action: string,
  extraData?: any,
) {
  try {
    ...
    if (action === "power-on") {
      // "Double-tap" Best Practice:
      // 1. Wake-on-LAN layer 2 (MAGIC PACKET) if MAC Address exists
      if (tv.macAddress) {
        await publishTVAction("255.255.255.255", "WOL", { mac: tv.macAddress });
      }
      // 2. Fallback layer 7 (API) via IP Address in case the TV was just in screen-off state
      await publishTVAction(tv.ipAddress, "POWER_ON");
    } else {
      let command = "";
      switch (action) {
        case "power-off":
      ...
      if (command) {
        ...
        await publishTVAction(tv.ipAddress, command as any, extraData);
      }
    }
