...
export async function createSession(adminId: string) {
  ...
  console.log(
    "Auth Debug: Created session token:",
    token.substring(0, 5) + "...",
  );
  return token;
}
...
export async function getAdminFromSession() {
  try {
    ...
    console.log(
      `Auth Debug: [getAdminFromSession] Raw Cookie Header: ${rawCookieHeader ? "Present" : "Missing"}`,
    );
    console.log(
      "Auth Debug: All Cookies received:",
      allCookies
    ...
    if (!token) {
      console.log(
        `Auth Debug: No token found in cookies. Server Time: ${new Date().toISOString()}`,
      );
      return null;
    }
    ...
    if (!session) {
      console.log(
        "Auth Debug: Invalid or expired session for token:",
        token.substring(0, 5) + "...",
      );
      ...
      if (expiredSession) {
        console.log(
          "Auth Debug: Session exists but expired at:",
    ...
    if (!admin) {
      console.log(
        "Auth Debug: Session valid but admin not found for ID:",
        session.adminId,
      );
      return null;
    }
